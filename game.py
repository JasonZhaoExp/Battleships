...
class Game:
    ...
    def generate_ships_and_mines(self):
        ...
        difficulty_index = SelectionMenu.get_selection(
        ...
        if difficulty == 'One Life':
            ...
            while True:
                try:
                    self.mineNumber = int(
                        input('Enter the number of mines (up to 50): '))
                    self.lives = int(
                        input('Enter the number of lives (up to 50): '))
                    self.shipNumber = int(
                        input('Enter the number of ships (up to 50): '))
        ...
        for i in range(self.shipNumber):
            while True:
                ship = ((random.randint(0, self.board.size - 1),
        ...
        for i in range(self.mineNumber):
            while True:
                mine = ((random.randint(0, self.board.size - 1),
                         random.randint(0, self.board.size - 1)))
    ...
    def start(self):
        ...
        while self.hits < self.shipNumber and self.lives > 0:
            ...
            while True:
                ...
                try:
                    ...
                    print("Choice outside of 0-9, try again")
        ...
        if self.hits == self.shipNumber:
            print(
                termcolor.colored(
                    "You sunk all the battleships! Congratulations!", "green"))
        elif self.lives == 0:
            print(termcolor.colored("You lost all your lives! Game over!"),
                  "red")
